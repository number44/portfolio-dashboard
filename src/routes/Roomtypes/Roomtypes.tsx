...
import { fetchRoomtypes } from '../../utils/fetching';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import { AiOutlinePlus } from 'react-icons/ai';
import { VscChromeClose } from 'react-icons/vsc';
interface PropsI {}
const RoomTypes = ({}: PropsI) => {
	const { data, isError, error, isLoading } = useQuery<RoomtypeI[], Error>(['roomtypes', { orderBy: 'id' }], fetchRoomtypes);
	...
	function compare(a: RoomtypeI, b: RoomtypeI) {
		if (a.id < b.id) {
			return -1;
		}
		if (a.id > b.id) {
	...
	const handleDelete = (id: number) => {
		if (confirm('Czy jesteś absolutnie pewien?')) {
			axios.delete(`/roomtypes/${id}`);
			qc.invalidateQueries('roomtypes');
		}
	};
